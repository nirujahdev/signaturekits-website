...
export default function SriLankaJerseysPage() {
  const [loading, setLoading] = useState(true);
  const [products, setProducts] = useState<any[]>([]);
  const collectionContent = getCollectionContent('sri-lanka-jerseys');
  ...
  const collectionName = collectionContent?.h1 || 'Sri Lanka Jerseys';
  
  const breadcrumbItems = [
    { name: 'Home', url: baseUrl },
    { name: collectionName, url: `${baseUrl}/collections/sri-lanka-jerseys` },
        ...
        <main className="container mx-auto px-6 py-12">
          <div className="mb-12">
            <h1 className="text-5xl font-semibold mb-4">
              {collectionContent?.h1 || 'Sri Lanka Jerseys'}
            </h1>
          ...
          {loading ? (
            ...
            <ProductList initialProducts={products} collection="sri-lanka-jerseys" />
          )}
